...
# Initialize auth_available flag
...
try:
    ...
    logger.error(f"❌ 認證模塊載入失敗: {e}")
    logger.error(f"❌ 錯誤類型: {type(e)}")
    import traceback
    logger.error(f"❌ 完整錯誤追蹤: {traceback.format_exc()}")
    auth_available = False

# Import and include redemption code routes (if not already loaded)
if 'redemption_router' not in locals():
    try:
