...
async def google_oauth_callback_get(
    ...
    """
    Google OAuth GET 回調處理
    
    處理來自 Google 的授權碼回調（標準 OAuth 流程）
    """
    # 構建請求對象  
    redirect_uri = "https://auto-trade-frontend-610357573971.asia-northeast1.run.app/auth/google/callback"
    ...
    # 調用簡化的callback邏輯 (不依賴資料庫)
    return await _google_oauth_callback_logic_simple(request)
...
async def google_oauth_callback_post(
    ...
    """
    Google OAuth POST 回調處理
    
    處理 POST 方式的授權碼（API 調用方式）
    """
    return await _google_oauth_callback_logic_simple(request)

async def _google_oauth_callback_logic_simple(request: GoogleCallbackRequest):
    """
    簡化的Google OAuth回調處理 (不依賴資料庫)
    """
    try:
        from .oauth import google_oauth, oauth_state_manager
        ...
        if not state_data:
            return {"error": "無效的授權狀態", "code": request.code[:10] + "..."}
        ...
        # 返回簡化的成功回應 (不創建JWT token)
        return {
            "success": True,
            "message": "Google OAuth驗證成功",
            "user_info": {
                "email": user_info.get("email"),
                "name": user_info.get("name"),
                "id": user_info.get("id")
            },
            "note": "資料庫功能未啟用，無法保存用戶數據或生成JWT token"
        }
        
    except Exception as e:
        logger.error(f"Google OAuth callback failed: {e}")
        return {"error": f"Google登入處理失敗: {str(e)}"}

async def _google_oauth_callback_logic(
    request: GoogleCallbackRequest,
    db: Session
):
    """
